...
export default function App() {
  ...
  const handlePost = async f => {
    const newRide = {
      ...
      cost_type: f.cost,
      price: f.cost==="free" ? 0 : parseFloat(f.amt),
      music_vibe: f.music,
      is_recurring: f.rec,
      recurring_days: f.rdays.join(","),
      is_live: true,
    };
    const {data, error} = await supabase.from("rides").insert([newRide]).select();
    if(error){
      console.error("Error saving ride:", error);
      notify("Something went wrong saving your ride");
    } else {
      // Also update local state so it shows immediately
      setRides(r=>[{id:data[0].id,drv:session?.user?.email?.split("@")[0]||"You",
        av:"ME",fr:f.fr,to:f.to,t:f.time,s:parseInt(f.seats),cost:f.cost,
    ...
    const fetchRides = async () => {
      ...
      if(!error && data && data.length > 0){
        const mapped = data.map(r=>({
          id: r.id,
          drv: r.driver_id?.slice(0,8)||"Driver",
          av: (r.driver_id?.slice(0,2)||"DR").toUpperCase(),
          fr: r.from_location,
          to: r.to_location,
          t: r.departure_time,
          s: r.seats,
          ...
          live: r.is_live,
          km: 48,
          pro: false, rec: r.is_recurring, 
          days: r.recurring_days ? r.recurring_days.split(",") : [],
          stu: false,
          music: r.music_vibe||"",
        }));
        setRides(prev=>[...mapped, ...prev]);
      }
    };
